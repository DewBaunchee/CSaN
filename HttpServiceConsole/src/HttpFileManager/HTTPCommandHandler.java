...
public class HTTPCommandHandler {
    ...
    private void createNestedDirectory(Path uri) throws IOException {
    ...
    public void view(String header, byte[] body, OutputStream out) throws IOException {
        ...
        fillListOfFiles(uri.toString(), listOfPaths, true);
    ...
    private void fillListOfFiles(String currentDir, List<String> list, boolean isCanonicalNeeded) {
        ...
        for (File current : files) {
            if (current.isDirectory()) {
                fillListOfFiles(current.getAbsolutePath(), list, isCanonicalNeeded);
            } else {
                if (isCanonicalNeeded) {
                    list.add(current.getAbsolutePath()
                            .substring(current.getAbsolutePath().indexOf(storage) + storage.length() + 1)
                            .replace("\\", "/"));
                } else {
                    list.add(current.getAbsolutePath());
                }
